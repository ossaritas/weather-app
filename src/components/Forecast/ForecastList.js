...
import For from './Forecast';
import ScrollToTop from '../ScrollToTop';

const Forecast = () => {
  const { city } = useParams(); //params to select city(query)
  const navigate = useNavigate();

  const { data } = useGetCityForecastQuery(city); // Data from weatherApi for selected city 5dayforecast  -- RTK Query
